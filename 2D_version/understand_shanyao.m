...
%探测器
Cx=9.1;Cy=-100; %mirror2左
Dx=9.1;Dy=-105; %mirror2右
km2=(Dy-Cy)/(Dx-Cx); %镜斜率
jm2=Cy-km2*Cx; %镜截距
...
%     lambda=[400e-9 475e-9 550e-9 620e-9 740e-9]; %j 遍历
    ...
    theta_zf0=asind(sind(phi)); %干涉0级
    theta_f1=asind(sind(phi)+lambda/d); %干涉-1级
    theta_f2=asind(sind(phi)+2*lambda/d); %干涉-2级
    theta_shan=4.3; %闪耀角
    ...
    [x4,y4,flag]=reflect(x1,y1,x_c0,y_c0,-(x_c0-center2(1))/(y_c0-center2(2)),km2,jm2,0,0,1);
    

    for j=1:numel(lambda)
        %grating -2
        kg_f2=tand(psi+90-theta_f2(j)); %衍射光斜率
        if kg_f2^2==Inf
            s=solve((x-center2(1)).^2+(y-center2(2)).^2==R2^2,x==x1,x,y);
        else
            s=solve((x-center2(1)).^2+(y-center2(2)).^2==R2^2, ...
        ...
        if numel(x_cf2)>1
            if x_cf2(1)>0
                x_cf2=x_cf2(1);y_cf2=y_cf2(1);
            else
                x_cf2=x_cf2(2);y_cf2=y_cf2(2);
            end
        end
        [x4,y4,flag]=reflect(x1,y1,x_cf2,y_cf2,-(x_cf2-center2(1))/(y_cf2-center2(2)),km2,jm2,0,0,1,lambda(j));
        ...
        [x4,y4,flag]=reflect(x1,y1,x_cf1,y_cf1,-(x_cf1-center2(1))/(y_cf1-center2(2)),km2,jm2,0,0,1,lambda(j));
        
        %grating 0
        kg_zf0=tand(psi+90-theta_zf0(j)); %衍射光斜率
        if kg_zf0^2==Inf
            s=solve((x-center2(1)).^2+(y-center2(2)).^2==R2^2,x==x1,x,y);
        else
            s=solve((x-center2(1)).^2+(y-center2(2)).^2==R2^2, ...
                (y-y1)==kg_zf0.*(x-x1),x,y);
        end
        x_czf0=double(s.x);
        y_czf0=double(s.y);
        if numel(x_czf0)>1
            if x_czf0(1)>0
                x_czf0=x_czf0(1);y_czf0=y_czf0(1);
            else
                x_czf0=x_czf0(2);y_czf0=y_czf0(2);
            end
        end
        [x4,y4,flag]=reflect(x1,y1,x_czf0,y_czf0,-(x_czf0-center2(1))/(y_czf0-center2(2)),km2,jm2,0,0,1,lambda(j));

        %grating +1
        kg_z1=tand(psi+90-theta_z1(j)); %衍射光斜率
        if kg_z1^2==Inf
            s=solve((x-center2(1)).^2+(y-center2(2)).^2==R2^2,x==x1,x,y);
        else
            s=solve((x-center2(1)).^2+(y-center2(2)).^2==R2^2, ...
        ...
        [x4,y4,flag]=reflect(x1,y1,x_cz1,y_cz1,-(x_cz1-center2(1))/(y_cz1-center2(2)),km2,jm2,0,0,1,lambda(j));
...
function [x3,y3,flag]=reflect(x1,y1,x2,y2,km,ko,jo,Cx,Dx,hua,lambda)
    ...
    if hua~=0
        ...
        if (hua==2) || (hua==3)
            plot([x1,x2],[y1,y2],'Color',color,'Linewidth',1,'LineStyle','--');%画入射光线
            plot([x2,x3],[y2,y3],'Color',color,'Linewidth',1,'LineStyle','--');%画反射光线
        else

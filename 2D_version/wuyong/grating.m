...
for i=1:Nn
    ...
    [xr2,yr2,flag]=reflect(Px(i),Py(i),xr1,yr1,0,0,40,10,20,0);
    %求交点
    s=solve(((x-x2)+k2.*(y-y2)).^2-2.*k2.*b2.*(x-x2)+2.*b2.*(y-y2)-b2.^2==0, ...
        (xr2-xr1).*(y-yr1)==(yr2-yr1).*(x-xr1),x,y);
    x_c0=double(s.x); 
    y_c0=double(s.y);
    kr02=pslope(x_c0,y_c0,x2,y2,k2,b2);
    [xr03,yr03,flag]=reflect(xr1,yr1,x_c0,y_c0,kr02,0,0,55,65,1);

    for j=1:numel(lambda)
        %grating -1
        s=solve(((x-x2)+k2.*(y-y2)).^2-2.*k2.*b2.*(x-x2)+2.*b2.*(y-y2)-b2.^2==0, ...
            (y-yr1)==tand(-theta_f1(j)+90).*(x-xr1),x,y);
        x_c1=double(s.x);  x_c1= x_c1(2);
        y_c1=double(s.y);  y_c1=y_c1(2);
        kr12=pslope(x_c1,y_c1,x2,y2,k2,b2);
        [xr13,yr13,flag]=reflect(xr1,yr1,x_c1,y_c1,kr12,0,-10,55,65,1,j);

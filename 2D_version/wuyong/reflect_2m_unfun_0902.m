...
plot([Ex,Fx],[Ey,Fy],'r','Linewidth',2);%探测器
Nn=5; %入射光线数
z=25:1:60; %设置离散点
%在平面镜上随机取Nn个点
...
for i=1:Nn
    ...
    if kr<=k_max && kr>=k_min
        ...
        [x4,y4]=symmetry(Px(i),Py(i),-1/kn,y3+x3/kn);
        kr2=(y4-y3)/(x4-x3);
        jr2=y3-kr2*x3;
        [x5,y5]=linecross(kr2,jr2,kd,jd);
        if x5>=Ex && x5<=Fx
            plot([x3,x5],[y3,y5],'Color',color,'Linewidth',1.5);
        else
            xx=1.04*x5;
            yy=kr2*xx+jr2;
            plot([x3,xx],[y3,yy],'Color',color,'Linewidth',1.5);
        end
    else

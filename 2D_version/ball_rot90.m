...
theta0=9.5; %闪耀角
lambda_shan=2.*d.*cosd(phi+theta0).*sin(theta0); %闪耀波长
...
for i=1:Nn
    ...
    x_c0=double(s.x); x_c0=x_c0(1);
    y_c0=double(s.y); y_c0=y_c0(1);
    [x4,y4,flag]=reflect(x1,y1,x_c0,y_c0,-(x_c0-center2(1))/(y_c0-center2(2)),km2,jm2,Cx,Dx,1);
    for j=1:numel(lambda)
        %grating -1
        kg_f1=tand(-theta_f1(j)); %衍射光斜率
        if kg_f1^2==Inf
            s=solve((x-center2(1)).^2+(y-center2(2)).^2==R2^2,x==x1,x,y);
        else
            s=solve((x-center2(1)).^2+(y-center2(2)).^2==R2^2, ...
        ...
        x_cf1=double(s.x); x_cf1=x_cf1(1);
        y_cf1=double(s.y); y_cf1=y_cf1(1);
        [x4,y4,flag]=reflect(x1,y1,x_cf1,y_cf1,-(x_cf1-center2(1))/(y_cf1-center2(2)),km2,jm2,Cx,Dx,1,j);
    end
end
axis equal
hold off;

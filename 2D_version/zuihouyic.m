...
R=101.6;
F=0.5*R;
t=linspace(-7.1808/180*pi,7.1808*pi/180,21); %直径25.4mm
center=[-R/2,-10];
X=R*cos(t)+center(1);
Y=R*sin(t)+center(2);
hold on;
...
Pt=t([mid-2,mid-1,mid,mid+1,mid+2]);
Nn=numel(Pt);%光线数
Px=R.*cos(Pt)+center(1);
Py=R.*sin(Pt)+center(2);
kr=0;
%平面镜1
psi=-90;  % 可见光范围83.3-86.8  85.6802
Ax=-30;Ay=-20; %平面镜左端
length=25;
...
for i=1:Nn
    ...
    %光栅
    ki=(y1-Py(i))/(x1-Px(i)); %算的是每个光自己的入射角
    phi=atand(ki)-psi-90;
    d=0.001/300; %3.33μm 即300条/mm
    %波长    紫     绿        红
    lambda=[380e-9 720e-9]; %j 遍历
%     lambda=[400e-9 475e-9 550e-9 620e-9 740e-9]; %j 遍历
    ...
    [x4,y4,flag]=reflect(x1,y1,x_c0,y_c0,-(x_c0-center2(1))/(y_c0-center2(2)),km2,jm2,10.8,10.8,1,lambda_shan);
    ...
    for j=1:numel(lambda)
        ...
        [x4,y4,flag]=reflect(x1,y1,x_cf1,y_cf1,-(x_cf1-center2(1))/(y_cf1-center2(2)),km2,jm2,10.8,10.8,1,lambda(j));
    end
end
axis equal
set(gca,'color','none');

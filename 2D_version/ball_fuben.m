...
for i=1:Nn
    ...
    %波长    紫     蓝       绿
    lambda=[400e-9 475e-9 550e-9]; %j 遍历
    theta_z1=asind(sind(phi)-lambda/d); %干涉+1级
    theta_f1=asind(sind(phi)+lambda/d); %干涉-1级
    theta_shan=4.3; %闪耀角
    lambda_shan=2.*d.*cosd(phi+theta_shan).*sind(theta_shan); %闪耀波长
    %闪耀光线方向
...
% 目标线段(cd)的斜率ko和截距jo,端点横坐标CxDx,i用于选颜色,hua 1画(画图可在后面加i参数) 0不画 2最强光

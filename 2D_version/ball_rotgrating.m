...
kr=0;
%平面镜1
psi=-86;  % 可见光范围83.3-86.8
midx=0;midy=-43.9;
length=25;
Ax=midx+length/2*cosd(psi+180);Ay=midy+length/2*sind(psi+180); %平面镜左端
Bx=midx+length/2*cosd(psi);By=midy+length/2*sind(psi); %平面镜右端
km1=(By-Ay)/(Bx-Ax); %镜斜率
jm1=Ay-km1*Ax; %镜截距
...
%探测器
Cx=6.65;Cy=-84; %mirror2左
Dx=6.65;Dy=-90; %mirror2右
km2=(Dy-Cy)/(Dx-Cx); %镜斜率
jm2=Cy-km2*Cx; %镜截距
...
%放杂散光2--挡板
da=0.7578;db=0.5285;
plot([0,40+da,40+da,nan,40+da,40+da],[-24+db,-24+db,-12+db,nan,0+db,yslit],'r-','Linewidth',2)
plot([130+da,160+da],[-44+db,-44+db],'r:','Linewidth',2)
syms x y;
for i=1:Nn
...
%     lambda=[400e-9 475e-9 550e-9 620e-9 740e-9]; %j 遍历
    ...
    [x4,y4,flag]=reflect(x1,y1,x_c0,y_c0,-(x_c0-center2(1))/(y_c0-center2(2)),km2,jm2,0,0,2,lambda_shan);
